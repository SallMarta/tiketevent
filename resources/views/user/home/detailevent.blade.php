  ...
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-12 col-lg-6 col-sm-12 text-center mb-4">
        <img src="{{asset('/event/poster_event/' .$eventdetail->poster_event)}}" class="img-detail-event">
      </div>
      <div class="col-md-12 col-lg-6 col-sm-12">
        <h1 class="text-center event-detail-title">{{ $eventdetail->nama_event }}</h1>
        ...
        <div class="tab-content" id="myTabContent">
          <div class="tab-pane fade show active mt-4" id="detail" role="tabpanel" aria-labelledby="detail-tab">
            ...
            <p>Kategori Event: {{ $eventdetail->nama_kategorievent }}</p>
            <p><span class="fas fa-calendar-day"></span> {{ date('j-F-Y', strtotime($eventdetail->tanggal_mulai)) }}</p>
            <p><span class="fas fa-map-marker-alt"></span> {{ $eventdetail->nama_tempat }}, {{ $eventdetail->alamat_event }}</p>
            <!-- <p><span class="fas fa-clock"></span> 17.00 s/d 23.00</p> -->
            <p class="mt-5">{{ $eventdetail->deskripsi }}</p>
            <div class="row">
              <div class="col-lg-12">
                <!--carousel-->
          ...
          <!-- <div class="container">
            ...
            <div class="row">
              <div class="col-md-4 col-sm-4 offline">
              ...
              <div class="col-md-8 col-sm-8 online">
                <h3 class="text-uppercase">tiket online</h3>
                <div class="">
                <form method="post" action="{{ url('/checkout/' .$eventdetail->id .'/event') }}" id="checkout">
                @method('post')
                @csrf
                  <table id="penjualan" class="table table-responsive table-hover">
                    ...
                    <tbody>
                      <tr class="tiketkategori" id="{{ $tiket->id }}">
                        <td>{{ $tiket->nama_kategoritiket }}</td>
                        <td>
                        <div value="" class="form-group">
                          <input id="qty" type="number" class="qty form-control" name="qty" min="0" placeholder="Qty">
                        </div>
                          <!-- <select value="" name="qty" class="qty form-control">
                            <option value="{{ $tiket->qty }}">{{ $tiket->qty }}</option>
                          </select> -->
                        </td>
                        <td>
                  ...
                  <button class="btn btn-success" style="float:right" value="submit" type="submit">Beli</button>
                </form>
                </div>
              </div>
          ...
          <div class="tab-pane fade mt-4" id="rules" role="tabpanel" aria-labelledby="rules-tab">
            <p>{{ $eventdetail->syarat_ketentuan }}</p>
          </div>
        </div>
      </div>
    </div>
